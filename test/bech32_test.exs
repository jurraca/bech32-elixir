defmodule Bech32Test do
  ...
  test "decoding" do
    assert {:ok, "ckb",
            <<1, 0, 248, 233, 196, 92, 241, 52, 177, 249, 178, 100, 1, 226, 254, 133, 46, 33, 214,
              246, 151, 234>>} === Bech32.decode("ckb1qyq036wytncnfv0ekfjqrch7s5hzr4hkjl4qs54f7e")
  end
  ...
  describe "BIP-173 valid SegWit test vectors" do
    test "BC1QW508D6QEJXTDG4Y5R3ZARVARY0C5XW7KV8F3T4" do
      {:ok, witver, data} =
        Bech32.segwit_decode("bc", "BC1QW508D6QEJXTDG4Y5R3ZARVARY0C5XW7KV8F3T4")
  ...
  describe "BIP-173 invalid SegWit test vectors" do
    test "invalid HRP (tc instead of tb or bc)" do
      assert {:error, :wrong_hrp} =
               Bech32.segwit_decode("bc", "tc1qw508d6qejxtdg4y5r3zarvary0c5xw7kg3g4ty")
    end

    test "invalid checksum" do
      assert {:error, :checksum_failed} =
               Bech32.segwit_decode("bc", "bc1qw508d6qejxtdg4y5r3zarvary0c5xw7kv8f3t5")
    end

    test "invalid witness version (31)" do
      assert {:error, :invalid_witness_version} =
               Bech32.segwit_decode("bc", "BC13W508D6QEJXTDG4Y5R3ZARVARY0C5XW7KN40WF2")
    end
    ...
    test "invalid program length for witness version 0" do
      assert {:error, :invalid_size} =
               Bech32.segwit_decode("bc", "BC1QR508D6QEJXTDG4Y5R3ZARVARYV98GJ9P")
    end
